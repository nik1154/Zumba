...
mongoose.connect(config1.database);
...
// SESSION
app.use(session({ cookie: { maxAge: 600000 },//10 mins 
                  secret: 'woot',
                  resave: false, 
                  saveUninitialized: false}));
...
//cache for media items expires in 10 minutes
...
//cache fot albums expires in 5 mins
...
//Passport Config
require('./config/cust_passport')(cust_passport);
//middleware to initialize
app.use(cust_passport.initialize());
app.use(cust_passport.session());
...
let main= require('./routes/index');
let index= require('./routes/main');
app.use('/main', main);
app.use('/', index);
app.use('/users',users);

// frontend will be loaded for every URI that isn't explicitly defined in the backend
// app.route('/*').get(function(req, res) { 
//     return res.sendFile(path.join(__dirname, 'views/pages/index')); 
// });
//google photo api starts here
...
app.get(
    ...
    passport.authenticate(
        'google', {failureRedirect: '/main', failureFlash: true, session: true}),
    (req, res) => {
      // User has logged in.
